...
async def create_admin(
    ...
    try:
        ...
        return await user_controller.create_user(
            db, admin, ip_address=request.client.host
        )
...
def deactivate_user(
    user_id: UUID, organization_id: UUID, db: Session = Depends(get_db)
):
    try:
...
def delete_user(
    ...
    try:
        ...
        return {
            "message": f"User {user_id} and associated data deleted successfully (Hard Delete)"
        }
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
